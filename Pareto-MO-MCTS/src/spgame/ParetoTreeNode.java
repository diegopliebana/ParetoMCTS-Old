...
public class ParetoTreeNode{
    ...
    public ParetoTreeNode(State state, Roller roller, TreePolicy treePolicy) {
        this(state, null, -1, roller, treePolicy);
    }

    public ParetoTreeNode(State state, ParetoTreeNode parent, int childIndex, Roller roller, TreePolicy treePolicy) {
        this.state = state;
        this.parent = parent;
        children = new ParetoTreeNode[state.nActions()];
    ...
    public boolean finishRollout(State rollerState, int depth, int action)
    {
        if(depth >= Controller.ROLLOUT_DEPTH)      //rollout end condition.
            return true;
    ...
    public int bestActionIndex(double target0, double target1) {
        ...
        for(int i = 0; i < myPA.size(); ++i)
        {
            ...
            double val = target0 * val0 + target1 * val1;
    ...
    {
        if(a_normalized)
            return pa.computeHV2(state.getValueBounds());
        else return
                pa.computeHV2();
