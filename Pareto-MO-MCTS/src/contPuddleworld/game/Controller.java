...
public abstract class Controller implements Runnable
{
    ...
    public final static int BOT_DELAY = 5;
    public final static double T = 1.0;
    public final static int MAX_TIME_STEPS = 1000;
    public final static int MAX_DISTANCE = 725; // SQRT(512^2 + 512^2) = 724.077

    public final static int ROLLOUT_DEPTH = 8; //8
    public static int MACRO_ACTION_LENGTH = 15; //15

    //public final static int ROLLOUT_DEPTH = 50;
    //public static int MACRO_ACTION_LENGTH = 1; //10
     ...
     * updated accordingly.
     *
     * @param a_cpwState A copy of the current game
     * @param a_timeDue The time the next move is due
     */
    public final synchronized void update(GameCPW a_cpwState,long a_timeDue)
    {
        //System.out.println("update");
        this.m_cpwState=a_cpwState;
            ...
            synchronized(this)
            {
                ...
                if(m_alive)
                {
                    ...
                    new Thread(){
                        public void run(){
                            int t = m_tick;
                            int move = getAction(m_cpwState, m_timeDue);
                            if(t == m_tick)
                            {
                                m_hasComputed=true;
                                m_lastMove=move;
     ...
     */
    public abstract int getAction(GameCPW a_cpwState, long a_timeDue);
