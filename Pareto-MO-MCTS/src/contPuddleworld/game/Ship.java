...
public class Ship extends GameObject
{
     ...
     */
    private final Color chassisColor = Color.cyan;     //Color.blue;
     ...
     */
    public Ship(GameCPW a_game, Vector2d a_startPos)
    {
        m_game = a_game;
    ...
    public void update(int a_actionId)
    {
        ...
        if(m_thrusting)
            m_potentialSpeed.add(d, Controller.T * 0.05 / 2);
        ...
        Goal g = m_game.getGoal();
        if(!g.collected)
        {
            boolean collected = g.checkCollected(this.s, this.radius);
            if(collected)
            {
                g.setCollected(true);
                m_game.goalReached();
    ...
    private int checkCollInPos(Vector2d a_collPoint)
    {
        ...
        if(m_game.getBoard().isOutsideBounds(xRound,yRound))
            return 1;

        if(m_game.getBoard().isObstacle(xRound, yRound))
        {
            if(m_game.getBoard().isCollisionUpDown(xRound,yRound))
            {
                return 1;
            }
            else
    ...
    {
        if(m_potentialPosition.x > m_game.getBoard().getMapChar().length-1)
        {
            m_potentialPosition.x = m_game.getBoard().getMapChar().length-1;
        }
        else if(m_potentialPosition.x < 0)
        {
            m_potentialPosition.x = 0;
        }
        else if(m_potentialPosition.y > m_game.getBoard().getMapChar()[0].length -1)
        {
            m_potentialPosition.y = m_game.getBoard().getMapChar()[0].length -1;
        }
     ...
     */
    public Ship getCopy(GameCPW a_game)
    {
